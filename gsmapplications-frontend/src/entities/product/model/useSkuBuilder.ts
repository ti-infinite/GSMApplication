...
// Product SKU builder — generic and reusable across features.
// Walks the category tree → resolves the SKU template → drives the ordered
// parameter cascade → filters master products by the built SKU prefix, with
// single-match / single-variety auto-selection. The `categoryOnlyFallback`
// option is the ONLY behavioral difference between consumers (see below).

export interface SkuBuilderOptions {
  /**
   * When a category has no SKU template, fall back to filtering products by
   * category (+ subcategory). Use this for catalogs that don't define SKU
   * templates yet. When false (default), a template with every parameter
   * chosen is required before products are listed.
   */
  categoryOnlyFallback?: boolean
}

export interface UseSkuBuilderResult {
...
export function useSkuBuilder(
  ...
  options:        SkuBuilderOptions = {},
): UseSkuBuilderResult {
  const { categoryOnlyFallback = false } = options
  ...
  // Two selection strategies, chosen by `categoryOnlyFallback`:
  // - false (template required): every SKU param must be chosen; a category
  //   without a template can never complete.
  // - true (category fallback): when the category has sub-categories, wait for
  //   one to be picked; then any SKU params (if a template exists) must be
  //   chosen — otherwise the category alone is enough.
  const needsSubcategory = !!(selectedCategory?.Children && selectedCategory.Children.length > 0)
  const allParamsSelected = categoryOnlyFallback
    ? !!effectiveCategory
      && (!needsSubcategory || !!selectedChildCategory)
      && rules.every(r => !!selectedParams[r.IdParameter])
    : rules.length > 0 && rules.every(r => !!selectedParams[r.IdParameter])
